    ...
    <div className="flex flex-col md:flex-row h-[calc(100vh-8rem)] relative pb-[56px] md:pb-0 -m-4 md:-m-8">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide flex flex-col gap-8 pb-4">
          {messages.map((msg) => (
            <div key={msg.id} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} max-w-[85%] ${msg.role === 'user' ? 'self-end' : 'self-start'}`}>
              ...
              {msg.role === 'user' ? (
                <div className="font-sans text-[14px] text-[#5C5C5C] uppercase tracking-[0.12em] animate-fade-in text-right">
                  {msg.content}
                </div>
              ) : (
                <div className="font-serif text-[20px] text-[#E8E6E0] font-normal">
      ...
      <div className="w-full md:w-[35%] bg-[#F5F0E8] flex flex-col p-4 md:p-8 overflow-y-auto h-[50vh] md:h-full">
        ...
        <div className="flex items-center gap-6 mb-8">
          ...
          <div>
            <h2 className="font-serif text-2xl font-bold text-[#1A1A1A] mb-1">AEGIS</h2>
        ...
        <div className="flex gap-2 mb-8">
          <button 
            ...
            className={`flex-1 py-2 font-mono text-[10px] tracking-widest uppercase border border-[#1A1A1A] transition-colors ${
            ...
            <div className="mb-8 flex-1">
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                <textarea
                  ...
                  className="w-full h-32 bg-[#EDE8E0] border border-[#C8C0B4] text-[#1A1A1A] p-4 font-serif text-lg resize-none focus:outline-none focus:border-[#1A1A1A] transition-colors placeholder:text-[#A8A094]"
                  onKeyDown={(e) => {
                ...
                <button 
                  ...
                  className="bg-[#1A1A1A] text-[#F5F0E8] py-4 font-mono text-xs tracking-widest uppercase hover:bg-[#C41E1E] transition-colors disabled:opacity-50"
                >
